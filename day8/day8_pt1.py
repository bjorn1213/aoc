...
for antenna in locations:
    for loc1, loc2 in combinations(locations[antenna], 2):
        ...
        antinode_row_idx = loc1[0] + row_delta
        antinode_col_idx = loc1[1] + col_delta
        antinode = (antinode_row_idx, antinode_col_idx)
        if check_antinode(antinode):
            antinodes.add(antinode)

        antinode_row_idx = loc2[0] - row_delta
        antinode_col_idx = loc2[1] - col_delta
        antinode = (antinode_row_idx, antinode_col_idx)
        if check_antinode(antinode):
            antinodes.add(antinode)
